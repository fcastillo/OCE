package  cod
{
	...
	public class CContenedorElemento extends CElementoBase
	{
		...
		private var size_ticker:int = 16;
		...
		public var weatherFirstTime : Boolean = true;
		...
		public function setID(val:int) {ID = val;}
...
import flash.display.Loader;
		...
		function actualizarMedidasPrueba()
		{
			...
			if (datos_elemento.width > getPadre().lienzo.contenedor.area_editable.width) datos_elemento.width = 225;
			if (datos_elemento.height > getPadre().lienzo.contenedor.area_editable.height) datos_elemento.height = 265;
			ajustarBotonesAcontenido();
			getPadre().getPanelPropiedades().insertPropGenericas(this);
		}
		/****************************************************************************************************/
		...
		{
			trace("[CContenedorElemento] - cambiarTiempoDeTipo()");
		...
		public function cargarFondoRSS()
		{
			...
			//if (true)
			{
				...
				this.datos_elemento.width = this.getAncho();
				this.datos_elemento.height = this.getAlto();
				if (this.getAncho() > 225)
				{
					ratio_ancho = 225/this.datos_elemento.fondo.width;
					this.setAncho(225);
					this.datos_elemento.fondo.width = 225;
					this.datos_elemento.width = this.getAncho();
					this.setXpos(0);
					this.x = 0;
		...
		{
			trace("[Flash] loadDataFromResource en CContenedorElemento");
		...
		public function loadContent(tipo:String,url:String,cajaTexto:TextField,cargaDirecta:Boolean)
		{
			...
			switch (tipo)
			{
				...
				case "video":
					...
					////añadido iñaki 2013-04-10
					var ratio_ancho, ratio_alto;
					trace("VIDEO inicial : "+video.getAncho()+", "+video.getAlto());
					if(video.getAncho() > video.getAlto())
					{
						trace("Es mas ancho que alto!");
						if (video.getAncho() > 225)
						{
							ratio_ancho = 225/video.getAncho();
							video.setAncho(225);
							//video.datos_elemento.fondo.width = 225;
							video.setXpos(0);
							video.x = 0;
							
							//Cambiamos alto proporcionalmente
							video.setAlto(video.getAlto()*ratio_ancho);
							//video.datos_elemento.fondo.height = this.getAlto();
							trace("Ajustado al ancho : "+video.getAncho()+", "+video.getAlto());
						}
						if (video.getAlto() > 265)
						{
							ratio_alto = 265/video.getAlto();
							video.setAlto(265);
							//video.datos_elemento.fondo.height = 265;
							video.setYpos(0);
							video.y = 0;
							
							//Cambiamos ancho proporcionalmente
							video.setAncho(video.getAncho()*ratio_alto);
							trace("Ajustado al alto : "+video.getAncho()+", "+video.getAlto());
							//video.datos_elemento.fondo.width = video.getAncho();
						}
					}
					else
					{
						trace("Es mas alto que ancho!");
						if (video.getAlto() > 265)
						{
							ratio_alto = 265/video.getAlto();
							video.setAlto(265);
							...
							video.setAncho(video.getAncho()*ratio_alto);
							trace("Ajustado al alto : "+video.getAncho()+", "+video.getAlto());
							//this.datos_elemento.fondo.width = video.getAncho();
						}
						if (video.getAncho() > 225)
						{
							ratio_ancho = 225/video.getAncho();
					...
					trace("VIDEO : "+video.getAncho()+", "+video.getAlto());
					////////////////////////////////////////////////////////////////
		...
		{					
			getPadre().getPanelPropiedades().btnRelacion.selected = true;
			trace("[CContenedorElemento] - handleData() - btnRelacion.selected = "+getPadre().getPanelPropiedades().btnRelacion.selected);
			...
			else if(getTipo() == "pasafotos")
			{
				trace("HANDLE DATA DE PASAFOTOS!");
			}
			...
			if(getTipo() != "video") this.datos_elemento.addChild(e.currentTarget.content);
			...
			getPadre().IDelemento_seleccionado = ID;
			getPadre().elemento_seleccionado = this;*/
			trace(" cambiando de tipo ========== "+cambiandoDeTipo);
			...
			if (estado == 0)//estamos cargando un elemento recién creado desde panel
			{
				...
				getPadre().elemento_seleccionado = this;
				insertarPestañaInferior();
				trace("Estado = 0 estamos cargando un elemento recién creado desde panel "+cambiandoDeTipo);
				if(getTipo() == "tiempo" && !cambiandoDeTipo)
				{
					trace("Ajustando el elemento tiempo al lienzo");
					var ratio_ancho:Number;
					var ratio_alto:Number;
					if (this.getAncho() > getPadre().anL)
					{
						ratio_ancho = getPadre().anL/this.getAncho();
						this.setAncho(getPadre().anL);
						this.datos_elemento.fondo.width = getPadre().anL;
						this.setXpos(0);
						this.x = 0;
					...
					if (this.getAlto() > getPadre().alL)
					{
						ratio_alto = getPadre().alL/this.getAlto();
						this.setAlto(getPadre().alL);
						this.datos_elemento.fondo.height = getPadre().alL;
						this.setYpos(0);
						this.y = 0;
				...
				//if(getTipo() == "tiempo" || getTipo() == "menu" || getTipo() == "rss" || getTipo() == "reloj")
				if(getTipo() == "menu" || getTipo() == "rss" || getTipo() == "reloj")
				{
					trace("Cargando elemento "+getTipo()+" creado desde panel -> w:"+this.getAncho()+", h:"+this.getAlto());
					var ratio_ancho:Number;
					var ratio_alto:Number;
					...
					trace("El swf del elemento "+getTipo()+" cargado => w:"+this.getAncho()+", h:"+this.getAlto());
					
					if (this.getAncho() > 225)
					{
						trace("ajustando a ancho");
						ratio_ancho = 225/this.getAncho();
						this.setAncho(225);
						this.datos_elemento.fondo.width = 225;
						this.setXpos(0);
						this.x = 0;
					...
					if (this.getAlto() > 265)
					{
						...
						ratio_alto = 265/this.getAlto();
						this.setAlto(265);
						this.datos_elemento.fondo.height = 265;
						this.setYpos(0);
						this.y = 0;
					...
					trace("Tras ajustar el "+getTipo()+" al lienzo => w:"+this.datos_elemento.fondo.width+", h:"+this.datos_elemento.fondo.height);
				}
				if (getTipo() == "imagen")
				{
					this.setAncho(e.currentTarget.content.width);
				...
				else if(getTipo() == "tiempo" || getTipo() == "menu" || getTipo() == "rss" || getTipo() == "reloj")
				{
					
				}
				else
				{
					trace("Reajustando not image!");
					trace("Handle data tenemos el tamaño del content -> "+e.currentTarget.content.width+", "+e.currentTarget.content.height);
					this.setAncho(e.currentTarget.content.width);
					this.setAlto(e.currentTarget.content.height);
					this.datos_elemento.fondo.width = e.currentTarget.content.width;
					this.datos_elemento.fondo.height = e.currentTarget.content.height;
					
					///añadido iñaki 10/04/2013 : 
					if (this.getAncho() > 225)
					{
						ratio_ancho = 225/this.getAncho();
						this.setAncho(225);
			...
			if(getTipo() == "tiempo")
			{
				...
				trace("weatherFirstTime = "+weatherFirstTime);
				if(weatherFirstTime && !cambiandoDeTipo)
				{
					trace("datos_elemento "+this.datos_elemento+" w : "+this.datos_elemento.width+" , h: "+this.datos_elemento.height);
					trace("this "+this+" w : "+this.getAncho()+" , h: "+this.getAlto());
					trace("loaded "+e.currentTarget.content+" w : "+e.currentTarget.content.width+" , h: "+e.currentTarget.content.height);
