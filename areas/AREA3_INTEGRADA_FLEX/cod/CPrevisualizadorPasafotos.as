package cod
{
	...
	public class CPrevisualizadorPasafotos extends CElementoBase
	{
		...
		{
			trace("Creando PREVISUALIZACCIÓN del elemento pasafotos -> w: "+pAncho+", h:"+pAlto+" x:"+pPosX+" y:"+pPosY);
			...
			//pasafotos.setConstants(0, pAncho*2, pAlto*2, 0);
			pasafotos.x = pPosX;
			pasafotos.y = pPosY;
			pasafotos.width = pAncho;
			pasafotos.height = pAlto;
			//pasafotos.setXpos(100);
			//pasafotos.setYpos(312);
		...
		/// funcion del previsualizador que espera hasta que haya fotos cargadas para hacerlse el resize
		/*function resizeDelPadasafotos(e:Event)
		{
			trace("resizeDelPadasafotos");
			if(contento.pasafotos.width != 0 && contento.pasafotos.height != 0)
			{
				trace("resizeDelPadasafotos no cero "+contento.pasafotos.width+", "+contento.pasafotos.height);
				this.removeEventListener(Event.ENTER_FRAME, resizeDelPadasafotos);
				this.setAncho(this.datos_elemento.fondo.width);
				this.setAlto(this.datos_elemento.fondo.height);
				contento.width = this.datos_elemento.fondo.width;
				contento.height = this.datos_elemento.fondo.height;
				this.width = contento.contenedor.width;
				this.height = contento.contenedor.height;
			}
		}*/
		///<summary>
