package  cod
{
	...
	public class CContenedorElemento extends CElementoBase
	{
		...
		public function setDimensiones()
		{
			...
			if(this == getPadre().elemento_seleccionado) insertarPestañaInferior();
		...
		public function insertarPestañaInferior():void
		{
			...
			if (!existe) this.addChild(pestaña);*/
...
import flash.display.Loader;
		...
		public function onElemento(e:MouseEvent)
		{
			...
			else
			{
				...
				trace("[CContenedorElemento] - onElemento() -> "+getTipo());
		...
		function actualizarMedidasPrueba()
		{
			...
			if (datos_elemento.height > getPadre().lienzo.contenedor.area_editable.height) datos_elemento.height = 170;
			ajustarBotonesAcontenido();
			getPadre().getPanelPropiedades().insertPropGenericas(this);
		}
		/****************************************************************************************************/
		...
		public function cargarFondoRSS()
		{
			...
			//Intentar que no supere los 225px de ancho!!
		...
		function handleData(e:Event)
		{
			...
			if(getTipo() == "imagen")
			{
				...
				*/
				if (estado == 0)
				{
					...
					trace("[CContenedorElemento] - handleData() -> imagen ("+this.getAncho()+", "+this.getAlto()+")");
				}
			}
			...
			if (estado == 0)//estamos cargando un elemento recién creado desde panel
			{
				...
				{
					trace("Cargando elemento creado desde panel -> w:"+this.getAncho()+", h:"+this.getAlto());
					var ratio_ancho:Number;
					...
					trace("El swf del elemento cargado => w:"+this.getAncho()+", h:"+this.getAlto());
					if (this.getAncho() > 225)
					{
					...
					if (this.getAlto() > 170)
					{
						...
						ratio_alto = 170/this.getAlto();
						this.setAlto(170);
						this.datos_elemento.fondo.height = 170;
						this.setYpos(0);
						//this.y = 0;
				...
				if (getTipo() == "imagen")
				{
					...
					if(this.getAncho() > getPadre().anL || this.getAlto() > getPadre().alL)
					{
						...
						//trace("Hemos ajustado la imagen al lienzo : ");
					}
				}
