...
{	
	...
	{		
		...
		public var changeLanguage:Boolean = false;
		...
		{
			//Variables lienzo
			anL = 378; //Ancho del lienzo
			alL = 800; //Alto del lienzo
		...
		function keyDownHandler( e:KeyboardEvent ):void
		{
			...
			//HEMOS COMENTADO LOS EVENTOS PARA LAS FLECHAS PORQUE INTERFIERE CON 
...
//			}
		...
		{
			trace("[Main] - desactivarContenedorMultiseleccion()");
		...
		public function duplicarElemento()
		{
			if (elemento_seleccionado != null)
			{
				...
				{					
					if (copia.control_reloj.getTipo() == "analog")
					{
						copia.setAncho(elemento_seleccionado.datos_elemento.width*.75);
						copia.cargarRelojAnalogico();
					}
		...
		function desactivarElementos()
		{
			...
			trace("[Main] - desactivarElementos() "+((!contenedorEliminado) && (elemento_seleccionMultiple != null)));
		...
		{
			trace("[Main] - vaciarContenedor -> lienzo.contenedor.numChildren = "+lienzo.contenedor.numChildren);
		...
		function deshacerSeleccionMultiple(x_init:Number,y_init:Number)
		{
			...
			trace("[CMain] - deshacerSeleccionMultiple("+x_init+", "+y_init+")");
		...
		{
			trace("[Main] - actualizarProfundidades_int()");
			//trace("****************actualizarProfundidades*******************");
			var indice:int;
			if (elemento_seleccionado != null) indice = lienzo.contenedor.getChildIndex(elemento_seleccionado);
			else if (elemento_seleccionMultiple != null) indice = lienzo.contenedor.getChildIndex(elemento_seleccionMultiple);
		...
		public function Align(accion:String,pos:Number,valor_max:Number)
		{
			...
			else if (accion == "right")
			{
				...
				//elemento_seleccionMultiple.x = xmin;
			}
			else if (accion == "top")
			{
		...
		{
			var urlRequest:URLRequest = new URLRequest("163.xml");
			...
			for (var k:int = 1; k <= elementos.length(); k++) //Va de profundidad 1 a profundidad = elementos.length()
			{
				for (var i:int = 0; i < elementos.length(); i++)
				{
					if (elementos[i].attribute("profundidad") == k)
					{
						...
						if (tipo == "texto") 
						{
							trace("Creando elemento texto con alpha = "+transparencia);
							...
							texto_aux.alpha = transparencia;//esto es trampita
						...
						else if (tipo == "imagen") 
						{
							...
							trace("[Main] - cargarElementosOrdenados() -> idRecurso="+elementos[i].attribute("ID"));
						...
						else if (tipo == "tiempo") 
						{
							...
							trace("[Main] - cargarElementosOrdenados() -> cuadro = "+cuadro+", lienzo="+lienzo+" listaElementos="+listaElementos);
						...
						{
							this.getPanelPropiedades().btnRelacion.selected = false;
							cuadro.setCandado(false);
							...
							// el ticker siempre es en una línea
		...
		public function getResourceURLFlash(resource_type:int, default_resource:String):void
		{
			trace("[oceEditor][Flash - Flex] call getResourceURL");
			try
			{
				this._resourceLibrary.getResourceURL(resource_type,default_resource);//id_recurso
			}
			catch (error:Error)
			{
		...
		{
			trace("[oceEditor][Flex - Flash] setSelectedResource. -> "+resource+" ñññ ");
