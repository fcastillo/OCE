package cod
{
	...
	public class CElementoVideo extends CElementoBase
	{
		...
		var videoCargado : Boolean = false;
		...
		public function setVideoMuted(val:Boolean) { muted = val; }
		...
		override public function load():int
		{
			...
			trace("[CElementoVideo] - load() -> numero de hijos del elemento : "+this.numChildren+" video="+video);
			if(video != null)
			{
		...
		function ns_onMetaData(item:Object):void 
		{
			...
			trace("[CElementoVideo] - ns_onMetaData() -> video("+getUrlDatos()+") item w: "+item.width+", h:"+item.height);
			if(redimensionado)
			{
			...
			trace("[CElementoVideo] - ns_onMetaData() -> video("+getUrlDatos()+") this w: "+this.width+", h:"+this.height);
