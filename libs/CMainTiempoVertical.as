package  {
	...
	public class CMainTiempoVertical extends MovieClip 
	{
		var item = new Array();
		var cod = "SPXX0052";//málaga
		var cod2 = "SPXX0061";//mallorca
		//http://weather.yahooapis.com/forecastrss?p=SPXX0061&u=f
		...
		var size_fecha:int = 17;
		var size_temp:int = 17;
		var color_fecha:uint = 0x5D5D5D;
		var color_temp:uint = 0x5D5D5D;
		var align_fecha:String = "center";
		var align_temp:String = "center";
		...
		public function CMainTiempoVertical() 
		{
			// constructor code
			//colorTexto1(1,"0x336699");
			//fuenteTexto(1,"Times new Roman");
			//getTomorrow("ITXX0081");
			//get2DaysAfterTomorrow(cod2);
			//loadXML(new XML('<ELEMENTO fecha_fuente="Verdana" temp_fuente="Verdana" alto="0.3125" ancho="0.3466666666666667" tipo="tiempo" dia="Hoy" ciudad="SPXX0087" pais="España" estilo="cartoon" posicion_textos="Arriba" fecha_formato="dd/mm"  fecha_color="6118749" fecha_negrita="true" fecha_cursiva="false" fecha_subrayado="false" temp_color="6118749" temp_negrita="true" temp_cursiva="false" temp_subrayado="false" profundidad="1" transparencia="1" cY="0.40625" cX="0.23733333333333334" path="tiempoAS3_vertical.swf"/>'));
		}
		...
		{		
			...
			fuente_fecha = datos.@fecha_fuente;
		...
		{
			//var formato_temp:TextFormat = new TextFormat();	
		//	formato_temp = txtTemperatura.getTextFormat();
		//	txtTemperatura.text = ".";
		//	txtTemperatura.setTextFormat(formato_temp);
		//	
		//	formato_fecha = new TextFormat();	
		//	formato_fecha = txtFecha.getTextFormat();
		//	txtFecha.text = ".";
		...
		//	formato_texto = txtTemperatura.getTextFormat();
		//	txtTemperatura.text = ".";
		//	txtTemperatura.setTextFormat(formato_texto);
		//	
		//	formato_texto = new TextFormat();	
		//	formato_texto = txtFecha.getTextFormat();
		//	txtFecha.text = ".";
		//	txtFecha.setTextFormat(formato_texto);	
			checkVisibility();
		}
		...
		public function showXML_cargaPaginas(e:Event)
		{
		...
		public function showXML(e:Event):void 
		{
			...
			var now : Date = new Date();
			var dia : String = now.getDate()<10 ?"0"+String(now.getDate()):String(now.getDate());
			var mes : String = (now.getMonth()+1)<10 ? "0"+String(now.getMonth()+1):String(now.getMonth()+1);
			//var dia : String = now.getDate().toString(); if(dia.length == 1) dia = "0"+dia;
			//var mes : String = (now.getMonth()+1).toString(); if(mes.length == 1) mes = "0"+dia;
			...
			txtTemperatura.y = 36;
			//txtFecha.width = 155;
			//txtFecha.width = icono.width;
			txtFecha.width = 106;
			txtFecha.height = 37;
			txtFecha.selectable = false;
			//txtTemperatura.width = 155;
			//txtTemperatura.width = icono.width;
			txtTemperatura.width = 130;
		...
		/*public function negritaTexto(instancia:int,op:Boolean):void
		{
			...
			if (instancia == 1) formato_texto = txtFecha.getTextFormat();
			else if (instancia == 2) formato_texto = txtTemperatura.getTextFormat();
			
			if(op)
			{
				if (instancia == 1) negrita_fecha = true;
				else negrita_temp = true;
				formato_texto.bold=true;
			}
			else 
			{
				if (instancia == 1) negrita_fecha = false;
				else negrita_temp = false;
				formato_texto.bold=false;	
			}	
					
			if (instancia == 1) txtFecha.setTextFormat(formato_texto);
			else if (instancia == 2) txtTemperatura.setTextFormat(formato_texto);
		}
		...
		public function cursivaTexto(instancia:int,op:Boolean):void
		{
			...
			if(op) 
			{
				if (instancia == 1) cursiva_fecha = true;
				else cursiva_temp = true;
				formato_texto.italic=true;
			}
			else
			{
				if (instancia == 1) cursiva_fecha = false;
				else cursiva_temp = false;
		...
		public function subrayadoTexto(instancia:int,op:Boolean):void
		{
			...
			if(op) 
			{
				if (instancia == 1) subrayado_fecha = true;
				else subrayado_temp = true;
				formato_texto.underline=true;
			}
			else 
			{
				if (instancia == 1) subrayado_fecha = false;
				else subrayado_temp = false;
				formato_texto.underline=false;		
			}
		...
		public function fuenteTexto(instancia:int,f):void
		{
			if (instancia == 1) this.fuente_fecha = f;
			else if (instancia == 2) this.fuente_temp = f;
		}
		
		public function cambiarFormatoFecha(nuevo_formato:String)
		{
			//trace("********** Tiempo - vertical - cambiarFormatoFecha************");
			//trace("nuevo_formato = ", nuevo_formato);
			//trace("formato = ", formato);
		...
		function checkVisibility()
		{
			...
			if (Math.round(difference_milliseconds/oneDay) > 0) 
			{ this.visible = false; }
			else 
			{ this.visible = true; }
